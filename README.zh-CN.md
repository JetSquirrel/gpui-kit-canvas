...
- **整理** – 一键把标题栏贴到顶部、侧边栏贴到前置边、状态栏贴到底部，把工具栏和面包屑排成标题栏下方的横带，对话框居中，通知落到后置角，其余按 16px 面板内边距重新排布。再按一次即可撤销。按钮的后半段可以选择正文的纵向位置（从顶部、居中、靠底部、均匀分布），该设置保存在窗口上并写入提示词。
- **对齐** – 右侧面板可以把所选组件左对齐、水平居中、右对齐、顶对齐、垂直居中、底对齐，或等距分布。多选时组件之间互相对齐；只选一个时与「整理」填充的正文区域对齐（若它位于容器内，则与该容器对齐）。
